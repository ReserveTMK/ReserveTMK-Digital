...
-   **Mentoring System**: In-house scheduling replacing Calendly. Features: mentor availability windows (weekly day/time slots with duration and buffer), public booking page (`/book/:userId` or `/book/mentor-{id}`) for external visitors, session lifecycle management (scheduled → confirmed → completed/cancelled/no-show), debrief logging with AI transcript analysis linking sessions to interactions, and mentoring delivery metrics in reports. Tables: `meetings` (extended with type/duration/bookingSource/notes/mentoringFocus/interactionId/coMentorProfileId/meetingTypeId/googleCalendarEventId), `mentor_availability` (userId/dayOfWeek/startTime/endTime/slotDuration/bufferMinutes), `mentor_profiles` (id/userId/mentorUserId/name/email/isActive/googleCalendarId), `meeting_types` (id/userId/name/description/duration/focus/color/isActive/sortOrder/createdAt). Admin mentor management: Ra manages a roster of mentors (Mentors tab), controls each mentor's availability (Availability tab with mentor selector), and sees all mentors' sessions (Sessions tab with mentor filter). Mentor assignment: Schedule dialog has mentor selector to assign sessions to any mentor in the roster. Co-mentor support: any session can have a co-mentor added/removed via dropdown on the session card. Meeting types (Calendly-style): admin defines reusable session types with name/duration/description/focus/color; 3 defaults seeded (Quick Chat 15min, Standard Session 30min, Deep Dive 60min); shown as selectable cards on public booking page. Google Calendar sync: sessions auto-create Google Calendar events with all attendees (mentor, co-mentor, mentee); co-mentor changes update attendees. Mentors without user accounts (e.g. Kim Beazley) use `mentor-{id}` as their booking identifier. Per-mentor Google Calendar sync: each mentor profile can specify a `googleCalendarId` for free/busy exclusion and event creation. Public API: `/api/public/mentoring/:userId/slots`, `/api/public/mentoring/:userId/book`, `/api/public/mentoring/:userId/meeting-types`. Mentor management API: `/api/mentor-profiles` (CRUD), `/api/meetings/all-mentors` (cross-mentor sessions), `/api/meeting-types` (CRUD). Debrief API: `POST /api/meetings/:id/debrief`. Reports include Mentoring subsection under Delivery. Default mentors seeded on first access: Ra Beazley (linked to user `54568936`) and Kim Beazley (pending login). Quick-setup endpoint: `POST /api/mentor-availability/quick-setup` seeds Mon–Fri 9–4 defaults.
