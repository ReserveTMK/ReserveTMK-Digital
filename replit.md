...
### Features
-   **Reporting Engine**: Funder-focused reporting across 9 sections (Reach, Delivery, Impact, Highlights, Value & Contribution, Benchmarks, Community Comparison, Tamaki Ora, Narrative). Reach counts ALL touchpoints (debriefs, meetings, events, emails, bookings, programmes) + monthly foot traffic. Delivery shows totalActivations headline (events + bookings + mentoring + partner meetings + programmes) with Workshops and Partner Meetings as first-class stats. Impact merges taxonomy breakdown + outcome movement + community spend with deduplicated milestones. Organisations Engaged subsection in Delivery lists groups with engaged/total member counts. People Featured subsection in Impact shows individuals with debriefs, milestones, stage progression, or new innovator status during the period (scoped by userId, respects community lens filtering). Highlights section with photo cards via Object Storage. Monthly foot traffic input dialog with trend chart. Growth metrics: mindset, skill, confidence (personal), bizConfidence (business). Tables: `monthlySnapshots` (foot traffic), `reportHighlights` (photo highlights). Report generation uses request deduplication and 60-second caching. Database indexes on key reporting columns for performance.
...
-   **Community Structure**: Cumulative 3-tier system (All Contacts / Our Community / Our Innovators) with shadcn Tabs view switching and flat list layout. Community page components are split across: `client/src/pages/contacts.tsx` (main page ~1100 lines), `client/src/components/community/inline-cells.tsx` (inline editable cells + shared config constants), `client/src/components/community/contacts-table.tsx` (table view), `client/src/components/community/contact-dialogs.tsx` (create/bulk upload/cleanup dialogs). Contact detail page (`contact-detail.tsx`) includes inline Connection Strength editor (5-level visual scale), Support Type editor (checkbox popover), promote/demote buttons, and tier badge. Tiers are inclusive: Community tab shows all community members including innovators; Innovators tab shows only innovators; All Contacts shows everyone. Sidebar uses "Community" dropdown with People (`/community/people`), Groups (`/community/groups`), and Ecosystems (`/community/ecosystems`) sub-routes. Old routes (`/contacts`, `/groups`, `/ecosystem`) redirect to new paths. Promote/demote endpoints (`POST /api/contacts/:id/promote`, `/demote`) cascade tier changes to linked groups via `groupMembers` table and `linkedGroupId`. Demote cascade checks remaining higher-tier members before demoting groups. Contacts have `isCommunityMember`, `isInnovator`, `supportType` (array), `connectionStrength` (known/connected/engaged/embedded/partnering), `movedToCommunityAt`, `movedToInnovatorsAt` fields. Contact roles defined in `CONTACT_ROLES` constant: Business Owner, Entrepreneur, Creative / Artist, Freelancer, Community Leader, Professional, Rangatahi / Student, Partner, Supporter, Other (with `roleOther` text field for custom descriptions). Groups have `isCommunity`, `isInnovator`, `movedToCommunityAt`, `movedToInnovatorsAt` fields. Groups table shows Name, Type, Community, Members, Contact columns. Group types defined in `GROUP_TYPES` constant: Business, Social Enterprise, Creative Movement, Community Initiative, Partner Organization, Funder, Other (with `organizationTypeOther` text field for custom descriptions). Both role and type filters show counts. Bulk actions include Set Role (contacts) and Set Organization Type (groups). Groups support promote/demote between tiers via ArrowUp buttons matching the People page pattern.
-   **Our Innovators**: A curated subset of contacts marked with `isInnovator` flag. On Community tab, amber lightbulb "Add" badge promotes to innovators inline. On Innovators tab, table shows Stage, Connection (5-level visual scale), and Support columns instead of Role. Community tab also shows Connection column. Support types: mentoring, workshop_skills, space, venue_hire, hot_desking, service_trade, paid_work, networking (`INNOVATOR_SUPPORT_TYPES` in schema). Connection strength: known, connected, engaged, embedded, partnering (`CONNECTION_STRENGTHS` in schema). Bulk promote/demote in edit mode. Shown as amber lightbulb badge on contact cards.
-   **Debriefs/Impact Tracking**: Components split across: `client/src/pages/debriefs.tsx` (orchestrator ~178 lines), `client/src/components/debriefs/review-view.tsx` (review/edit), `client/src/components/debriefs/calendar-debrief-tab.tsx` (event queue), `client/src/components/debriefs/weekly-debrief-tab.tsx` (weekly summaries with refresh capability), `client/src/components/debriefs/new-debrief-dialog.tsx`, `client/src/components/debriefs/manual-update-dialog.tsx`, `client/src/components/debriefs/shared.tsx` (types/constants). Manual impact tags use taxonomy category selector for database persistence. Weekly summaries support in-place refresh of draft data. Outstanding debriefs count is week-scoped with separate all-time backlog indicator. Interaction AI analysis syncs all 8 metric scores (mindset, skill, confidence, bizConfidence, systemsInPlace, fundingReadiness, networkStrength, communityImpact) to contacts using merge strategy.
