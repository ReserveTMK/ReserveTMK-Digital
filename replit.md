...
-   **Regular Booker Portal**: Self-service portal for regular bookers with magic link authentication, package status, and booking flows. Supports group-level portal links (`isGroupLink` on bookerLinks) so orgs can share one link. Pricing is shown during booking selection (rates, discounts, agreement coverage). Auto-suggestions surface contacts with venue_hire/hot_desking support types or active agreements.
-   **Regular Bookers Page**: Dedicated page at `/regular-bookers` (under Delivery nav) with table view showing booker name, group/org, agreement, package balance, pricing tier, portal link status, and account status. Each row has inline Generate Link / Copy Link, Edit, and Delete actions. Portal links are generated from the table (not from the create form). The create form is simplified: no login email or portal access section. A portal link is auto-generated on creation and shown in a success dialog with clipboard copy.
