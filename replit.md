...
-   **Spaces Hub**: Unified hub at `/spaces` with 5 tabs: Calendar (venue + desk availability grid), Venue Hire (booking management with kanban/list views), Hot Desking (desk availability cards), Resources (manage venues and desks with two sub-tabs: Venues and Desks), Bookers (regular bookers filtered to venue hire + hot desking categories). Venue Hire Settings (gear icon) contains: Reminders, Survey, Portal, Xero. Resource management components extracted to `client/src/components/spaces/resources-tab.tsx`. Each venue card is expandable with tabbed sections for Details (name/description/capacity) and Availability Schedule (per-day operating hours). Instructions are managed at the location (spaceName) level — one set of instructions shared by all venues in that location, displayed as a collapsible section under each location header in the Venues sub-tab. The `venue_instructions` table has a `spaceName` column for location-level association (venueId kept for backward compat). The Desks sub-tab contains desk resources and hot desking hours. Booking reminders (configured in Venue Hire Settings → Reminders) replace the old after-hours reminders — they send location instructions for all venue hire bookings. The `bookings` table has a `locationAccess` text array column for per-booking location access control. On the booking detail view, admins can select which locations a booker has access to via checkboxes. The `booking_reminder_settings` table stores the enabled toggle and send timing. Bookings support multi-venue selection via `venueIds` integer array column (with legacy `venueId` kept for backwards compat). The booking form uses checkboxes instead of a dropdown, so users can tick multiple venues for a single booking.
...
-   **Booking Categories**: Unified framework for Venue Hire, Hot Desking, and Gear Booking, managed via bookable resources and linked agreements. Agreements (Memberships/MOUs) have category checkboxes for Venue Hire, Hot Desking, and Gear. Hot desking hours are driven by the operating hours settings in the Desks sub-tab. Booking reminders send location instructions before venue hire bookings.
