...
-   **Mentoring System**: In-house scheduling replacing Calendly. Features: mentor availability windows (weekly day/time slots with duration and buffer), public booking page (`/book/:userId`) for external visitors, session lifecycle management (scheduled → confirmed → completed/cancelled/no-show), debrief logging with AI transcript analysis linking sessions to interactions, and mentoring delivery metrics in reports. Tables: `meetings` (extended with type/duration/bookingSource/notes/mentoringFocus/interactionId), `mentor_availability` (userId/dayOfWeek/startTime/endTime/slotDuration/bufferMinutes). Public API: `/api/public/mentoring/:userId/slots`, `/api/public/mentoring/:userId/book`. Debrief API: `POST /api/meetings/:id/debrief`. Reports include Mentoring subsection under Delivery with session counts, hours, mentee stats, focus area breakdown, and debrief rates. Google Calendar free/busy sync: slot generation checks Google Calendar busy periods and removes conflicting slots. Quick-setup endpoint: `POST /api/mentor-availability/quick-setup` seeds Mon–Fri 9–4 defaults. Ra Beazley (user `54568936`) has availability seeded.
