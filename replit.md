...
-   **Venue Hire System**: Manages venue hires with automated workflows, conflict checking, and post-hire surveys. Post-completion dialog triggers next-step actions (invoice + mark served) with `servedAt` and `invoiceRequested` tracking. UI labels say "Venue Hire" throughout; routes (`/bookings`), API (`/api/bookings`), and DB (`bookings` table) unchanged.
