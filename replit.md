...
-   **Spaces Hub**: Unified hub at `/spaces` with 5 tabs: Calendar (venue + desk availability grid), Venue Hire (booking management with kanban/list views), Hot Desking (desk availability cards), Resources (manage venues and desks with sub-tabs, includes default pricing), Bookers (regular bookers filtered to venue hire + hot desking categories). Venue Hire Settings (gear icon) contains: Venue Instructions, After Hours, Survey, Xero. Resource management components extracted to `client/src/components/spaces/resources-tab.tsx`. Each venue card is expandable into a management hub with tabbed sections for Details (name/description/capacity), Availability Schedule (per-day operating hours with open/closed toggles), and Instructions (per-venue instructions with type categorization). The `venues` table has an `availabilitySchedule` JSONB column for day-of-week hours, and `venue_instructions` has a nullable `venueId` column (null = general/all venues). The conflict-checking endpoint uses venue-specific availability instead of hardcoded 8am-5pm. Bookings support multi-venue selection via `venueIds` integer array column (with legacy `venueId` kept for backwards compat). The booking form uses checkboxes instead of a dropdown, so users can tick multiple venues for a single booking. The "Both (Boardroom + Workshop)" venue has been deactivated — users now select individual venues.
...
-   **Booking Categories**: Unified framework for Venue Hire, Hot Desking, and Gear Booking, managed via bookable resources and linked agreements. Agreements (Memberships/MOUs) have category checkboxes for Venue Hire, Hot Desking, and Gear. Hot desking hours are driven by the Operating Hours settings in Resources tab (no longer hardcoded). The operating hours also determine after-hours venue hire flagging.
