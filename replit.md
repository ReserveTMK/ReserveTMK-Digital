...
-   **Community Structure**: Hierarchical 3-tier system (All Contacts / Our Community / Our Innovators) with summary cards landing view, drill-down table/list views, quick promote buttons (ArrowUp), and cascade logic. Sidebar uses "Community" dropdown with People (`/community/people`), Groups (`/community/groups`), and Ecosystems (`/community/ecosystems`) sub-routes. Old routes (`/contacts`, `/groups`, `/ecosystem`) redirect to new paths. Promote/demote endpoints (`POST /api/contacts/:id/promote`, `/demote`) cascade tier changes to linked groups via `groupMembers` table. Contacts have `isCommunityMember`, `isInnovator`, `movedToCommunityAt`, `movedToInnovatorsAt` fields. Groups have `isCommunity`, `isInnovator`, `movedToCommunityAt`, `movedToInnovatorsAt` fields.
-   **Our Innovators**: A curated subset of contacts marked with `isInnovator` flag. Quick-promote via ArrowUp button in table/list views; bulk promote/demote in edit mode. Shown as amber lightbulb badge on contact cards.
